...
const source = fs.readFileSync(path.join(__dirname, '../omniossendlogs.js'), 'utf8');
const plugin = 'omniossendlogs', readAction = 'checkExportCapabilities';
...
test('export requires Agent Console rights and forwards verified rights', () => {
    for (const rights of [0, 8, 256, 16]) {
        const h = setup(true, rights);
        h.obj.serveraction({pluginaction: 'triggerExport', nodeid, window: '60m', rights: 0xFFFFFFFF}, h.browser, h.webserver);
        assert.equal(h.sent.length, rights === 16 ? 1 : 0);
        if (h.sent.length) assert.equal(h.sent[0].rights, 16);
    }
});
...
test('different exports receive busy instead of sharing a success result', () => {
    ...
    h.obj.serveraction({pluginaction: 'triggerExport', nodeid, window: '120m', clientRequestId: 'b'}, h.browser, h.webserver);
    assert.equal(h.sent.length, 1);
    assert.equal(h.replies.at(-1).data.clientRequestId, 'b');
    assert.equal(h.replies.at(-1).data.status, 'error');
    h.obj.serveraction({pluginaction: 'exportResult', requestId: h.sent[0].requestId, success: true}, h.agent, h.webserver);
    assert.equal(h.replies.at(-1).data.clientRequestId, 'a');
    assert.equal(h.replies.at(-1).data.status, 'success');
});
test('forwards only allowed window literals', () => {
    for (const window of ['30m', '60m', '120m', '60m; touch /tmp/pwned', ['60m'], 60, {}]) {
        const h = setup();
        h.obj.serveraction({pluginaction: 'triggerExport', nodeid, window}, h.browser, h.webserver);
        assert.equal(h.sent[0].window, ['30m','60m','120m'].includes(window) ? window : undefined);
    }
});
